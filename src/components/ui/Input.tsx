    ...
    return (
      <label className="group flex w-full flex-col gap-2">
        ...
        <input
          ref={ref}
          id={fieldId}
          className={cn(fieldBase, error && "border-danger focus:border-danger focus:shadow-[0_1px_0_0_var(--color-danger)]", className)}
          aria-invalid={!!error}
...
export const TextArea = forwardRef<HTMLTextAreaElement, TextAreaProps>(
  function TextArea({ className, label, hint, error, id, rows = 4, ...rest }, ref) {
    const fieldId = id || rest.name;
      ...
      <label className="group flex w-full flex-col gap-2">
        ...
        <textarea
          ...
          className={cn(
            fieldBase,
            "resize-y min-h-[7rem]",
            error && "border-danger focus:border-danger focus:shadow-[0_1px_0_0_var(--color-danger)]",
            className,
          )}
          aria-invalid={!!error}
          aria-describedby={error ? `${fieldId}-error` : hint ? `${fieldId}-hint` : undefined}
